...
def main():
    ...
    def balance_test_data(data):
        atelectasis = [img for img in data if 'Atelectasis' in img['labels'].split('|')]
    ...
    atelectasis = [img for img in train_val_data if 'Atelectasis' in img['labels'].split('|')]
    ...
    def save_images_to_subdirs(images, split_name):
        split_dir = os.path.join('./only_PA', split_name)
...
def divide_test_directory():
    # Paths
    test_dir = './only_PA/test'
    output_dirs = ['./only_PA/test_1', './only_PA/test_2', './only_PA/test_3']
