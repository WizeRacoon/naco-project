...
def genetic_clustering(grey_values, num_clusters, initial_population, generations=10, crossover_prob=0.8, mutation_prob=0.1):
    ...
    for i in range(generations):
        ...
        # Population selection with elitism
        fitness_scores.sort(key=lambda x: x[0])
        selected = [x[1] for x in fitness_scores[:max(2, population_size // 2)]]  # Ensure at least 2 individuals

        # Handle case where selected has fewer than 2 individuals
        if len(selected) < 2:
            print("[Warning] Not enough individuals for crossover. Skipping genetic clustering.")
            return best_palette
